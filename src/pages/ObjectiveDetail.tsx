      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="mb-12">
          ...
          <div className="text-sm font-medium tracking-widest text-gray-400 uppercase">
            {parseLocal(goal.startDate).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }).replace('.', '')} &mdash; {parseLocal(goal.endDate).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }).replace('.', '')}
          </div>
        </div>

        <GoalTasks goalId={goal.id} tasks={tasks} onUpdate={reload} />

        <GoalCalendar goal={goal} marks={marks} onUpdate={reload} />
